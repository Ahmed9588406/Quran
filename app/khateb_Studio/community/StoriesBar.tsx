...
export default function StoriesBar({ onStoryClick, onCreateStory }: StoriesBarProps) {
  ...
  const fetchStories = async () => {
    try {
      ...
      console.log('Khateb Stories API response status:', response.status);
      ...
      const data = await response.json();
      console.log('Khateb Stories API response data:', data);
      ...
      const transformedStories: Story[] = usersWithStories.map((user: any) => {
        ...
        const userStories: ApiStory[] = (user.stories || []).map((story: any) => ({
          ...
          viewed: story.viewed_by_me || story.viewed || false,
        }));
      ...
      // Sort stories: unseen stories first
  ...
  const handleStoryClick = (story: Story) => {
    ...
    if (story.isYourStory) {
      return; // Handled by handleYourStoryClick
  ...
  // Check if "Your story" has active stories
