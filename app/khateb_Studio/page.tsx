...
import { ChevronLeft, ChevronRight, Upload, Pencil } from "lucide-react";
...
import CreateModal from "./CreateModal"; // added import
import UploadModal from "./UploadModal"; // new
import UploadSuccessModal from "./UploadSuccessModal"; // new
import GoLiveModal from "./GoLiveModal"; // NEW: import go live UI
import AudioModal from "./Audio_modal"; // NEW: import audio/live-room UI
import MicrophoneSettingsModal from "./MicrophoneSettingsModal"; // NEW import
...
function StatCard({ title, subtitle, icon }: StatCardProps) {
  return (
    <div className="flex items-center gap-2 p-2 bg-[#FFF9F3] rounded-lg w-[268px] h-[93px]">
...
// Calendar Component
function Calendar() {
  const [currentDate] = useState(new Date(2021, 2, 13)); // March 13, 2021 as shown in design
  ...
  const remainingDays = 42 - totalDaysShown; // 6 rows * 7 days
    ...
    <div className="bg-white rounded-[20px] p-4 shadow-md w-full">
      ...
      <div className="grid grid-cols-7 gap-2">
        {allDays.slice(0, 42).map((dayObj, index) => (
          <div
            key={index}
            className={`w-[27px] h-[27px] flex items-center justify-center text-[15px] font-medium rounded-full
              ${!dayObj.isCurrentMonth ? "text-[#8A1538] opacity-20" : "text-[#8A1538]"}
              ${"isSelected" in dayObj && dayObj.isSelected ? "bg-[#CFAE70] text-white" : ""}
            `}
          >
            {dayObj.day}
          </div>
...
export default function KhateebStudioPage() {
  ...
  const [isGoLiveOpen, setIsGoLiveOpen] = useState(false); // NEW state for Go Live modal
  const [isAudioOpen, setIsAudioOpen] = useState(false); // NEW or existing state
  const [isMicSettingsOpen, setIsMicSettingsOpen] = useState(false); // NEW state

  // Listen for global event to open audio modal (emitted by AudioSettingsModal fallback)
  useEffect(() => {
  ...
  // Listen for global event to open mic settings (emitted by AudioSettingsModal fallback)
  useEffect(() => {
  ...
  return (
    <div className="h-screen bg-white flex flex-col overflow-hidden">
      ...
      <KhatebNavbar
        ...
        onProfileClick={(e) => {
          // find the button element (avatar) and store its bounding rect
          const btn = document.getElementById('khateb-avatar-btn');
          const rect = btn ? btn.getBoundingClientRect() : (e.currentTarget as HTMLElement).getBoundingClientRect();
          setProfileAnchor(rect);
          setIsProfileOpen(true);
        }}
        onCreateClick={(e) => {
          // compute anchor rect for create button (same pattern as profile)
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="h-full p-6 flex flex-col">
            ...
            <div className="sticky top-0 z-30 bg-white/95 backdrop-blur-sm -mx-6 px-6 py-12 border-b border-gray-100">
                <div className="flex items-center justify-between mb-0">
                    ...
                    <div className="flex items-center gap-4">
                        <button className="w-12 h-12 flex items-center justify-center border border-[#E3E3E3] rounded-full hover:bg-gray-50">
            ...
            {/* Page content starts below the sticky header */}

            {/* Stat Cards Row */}
            <div className="flex flex-wrap gap-35 mb-6 shrink-0">
              <StatCard
              ...
              <StatCard
                ...
                icon={
                  <div className="w-[60px] h-[60px] rounded-ful flex items-center justify-center">
                    <Image
      ...
      <KhatebProfileModal isOpen={isProfileOpen} onClose={() => setIsProfileOpen(false)} anchorRect={profileAnchor} />
      ...
      <CreateModal
        ...
        onOpenUpload={() => {
          // show upload modal and close create menu
          setIsUploadOpen(true);
          setIsCreateOpen(false);
        }}
        onOpenGoLive={() => {
          // open the Go Live setup modal (which will then open Audio_modal)
          setIsGoLiveOpen(true);
          setIsCreateOpen(false);
        }}
      />

      {/* Upload Modal (separate file) */}
      <UploadModal
        open={isUploadOpen}
        onClose={() => setIsUploadOpen(false)}
        onSuccess={() => {
          setIsUploadOpen(false);
          // compute position near audio modal if present
          const audioEl = typeof document !== "undefined" ? document.getElementById("audio-modal-root") : null;
          const rect = audioEl ? audioEl.getBoundingClientRect() : null;
          setUploadSuccessAnchor(rect);
          // show success modal after a tick to ensure rect measured
      ...
      {/* Go Live Modal (centered) */}
      <GoLiveModal
        open={isGoLiveOpen}
        onClose={() => setIsGoLiveOpen(false)}
        onStartLive={() => {
          // close the GoLive setup modal and open the bottom audio/live room
      ...
      {/* Microphone settings modal (opened from AudioSettingsModal) */}
      <MicrophoneSettingsModal
        open={isMicSettingsOpen}
