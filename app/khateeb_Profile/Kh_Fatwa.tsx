...
function FatwaCard({ entry }: { entry: FatwaEntry }) {
  const [isFollowing, setIsFollowing] = useState(false);
  const [isTogglingFollow, setIsTogglingFollow] = useState(false);

  const handleFollowClick = async () => {
    if (!entry.author.id || isTogglingFollow) return;
    
    const prev = isFollowing;
    setIsFollowing(!prev);
    setIsTogglingFollow(true);
    
    const success = await toggleFollowUser(entry.author.id, prev);
...
export default function Fatwa({ entries }: FatwaProps) {
  return (
    <div className="space-y-4">
      {entries.map((entry) => (
        <FatwaCard key={entry.id} entry={entry} />
      ))}
    </div>
  );
}
