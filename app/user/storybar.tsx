    ...
    try {
      ...
      const transformedStories: Story[] = usersWithStories.map((user: any) => {
        ...
        if (avatarUrl && !avatarUrl.startsWith('http')) {
          avatarUrl = `http://apisoapp.twingroups.com${avatarUrl}`;
        }
        if (!avatarUrl) {
        ...
        const userStories: ApiStory[] = (user.stories || []).map((story: any) => ({
          ...
          media_url: story.media_url?.startsWith('http') 
            ? story.media_url 
            : `http://apisoapp.twingroups.com${story.media_url}`,
      ...
      console.log('Transformed stories:', transformedStories.length, 'users with stories');
      ...
      if (currentUserAvatar && !currentUserAvatar.startsWith('http')) {
        currentUserAvatar = `http://apisoapp.twingroups.com${currentUserAvatar}`;
      }
      if (!currentUserAvatar) {
        currentUserAvatar = '/icons/settings/profile.png';
      }

      // Add "Your story" at the beginning, then all following stories
